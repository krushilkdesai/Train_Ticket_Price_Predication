...
model_path = os.path.join(root_dir, "random_forest_regression_model.pkl")
...
def predict():
    ...
    if not Train or not Source or not Destination:
        return render_template('index.html', error='Please choose train, source, and destination.')
    ...
    prediction = model.predict([[
        Train_Garib_Rath, Train_Humsafar_Express, Train_Jan_Shatabdi_Express,
        Train_Mahamana_Express, Train_Rajdhani_Express, Train_Sampark_Kranti_Express,
    ...
    if output < 0:
        return render_template('index.html', pred='Sorry, prediction returned invalid price.')
